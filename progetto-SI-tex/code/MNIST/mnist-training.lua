----------------------------------------------------------------------
--Training di una Convolutional Neural Network sul dataset MNIST 
--Tipico problema di classificazione a 10 classi 
...
--   -n,--network       (default "logs/mnist.net.old")          reload pretrained network
...
local opt = lapp[[
   ...
   -v,--visualize     (default 0)       let the user visualize all the tested images with predictions
]]



if(opt.maxIter==7) then print 'gay' os.exit() end
...
if opt.network == '' then
   ...
   if opt.model == 'convnet' then
    ...
    --  model:add(nn.SpatialConvolutionMap(nn.tables.random(3,32,1),5,5))
      ...
      model:add(nn.SpatialConvolutionMM(32, 64, 5, 5))  -- qui troviamo immagine 4x4
      --      model:add(nn.SpatialConvolutionMap(nn.tables.random(32,64,1),5,5))

      model:add(nn.Tanh())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))
      -- stage 3 : standard 2-layer MLP:
      model:add(nn.Reshape(64*2*2))  -- alla fine immagine 2x2 addirittura
   ...
   for t = 1,dataset:size(),opt.batchSize do
    ...
    --here we visualize all the inputs tested and we label them with the correspective predictions
     ...
     for j=1,(k-1) do
       ...
       gfx.image(img,{legend=label,zoom=5})
...
print ('training completed. Percentige: '..complete..'%')
