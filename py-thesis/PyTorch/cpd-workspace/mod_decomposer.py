...
def estimate_cp_ranks( layer, compression_factor=0):
    ...
    if compression_factor:
        rank, _= _choose_compression(layer, ranks, compression_factor, flag='cpd')
    return rank
    

# NB. ideally this should not even have the rank option and be more 
#     intuitive to use. 
def pytorch_cp_layer_decomposition(layer, rank, offline=False, filename=''):
    """ 
...
def pytorch_cp_layer_decomposition_BN( layer, rank, offline=False, filename=''):
    """ 
...
def FC_SVD_compression( layer, trunc=15):
    """
    ...
    weights1, weights2 = _SVD_weights(
...
def conv1x1_SVD_compression( layer, trunc=15):
    """
    ...
    weights1, weights2 = _SVD_weights(W.T, trunc)
...
# DA FARE 
def _cp_xavier_conv_layer( layer, rank):
