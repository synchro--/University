...
from keras.layers import Conv3D, SeparableConv2D, MaxPooling2D, BatchNormalization
...
## 1
# model.add(SeparableConv2D(32, (3, 3), padding='same', input_shape=x_train.shape[1:]))
model.add(SeparableConv2D(rank1, kernel_size=(3, 1),
          padding='same', input_shape=x_train.shape[1:]))
model.add(BatchNormalization())
model.add(SeparableConv2D(32, kernel_size=(1, 3)))
model.add(BatchNormalization())
...
model.add(SeparableConv2D(rank1, kernel_size=(3, 1)))
model.add(BatchNormalization())
model.add(SeparableConv2D(64, kernel_size=(1, 3)))
model.add(BatchNormalization())
model.add(Activation('relu'))

## 5
# model.add(SeparableConv2D(64, (3, 3)))
model.add(SeparableConv2D(rank2, kernel_size=(3, 1)))
model.add(BatchNormalization())
model.add(SeparableConv2D(64, kernel_size=(1, 3)))
model.add(BatchNormalization())
model.add(Activation('relu'))
...
## 7
model.add(SeparableConv2D(rank1, kernel_size=(5, 1)))
model.add(BatchNormalization())
model.add(SeparableConv2D(512, kernel_size=(1, 5)))
model.add(BatchNormalization())
model.add(Activation('relu'))

## 8 Classifier
model.add(SeparableConv2D(num_classes, kernel_size=(1, 1), activation=None))
model.add(Flatten())
